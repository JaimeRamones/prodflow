...
serve(async (req) => {
  ...
  try {
    ...
    if (!shipment_ids || !format) throw new Error("Faltan parámetros: shipment_ids o format.");
    ...
    // Devolvemos la respuesta de Mercado Libre directamente al cliente
    return new Response(meliResponse.body, {
        headers: { ...corsHeaders, 'Content-Type': contentType }
    });

  } catch (error) {
    console.error("Error en get-ml-labels:", error);
