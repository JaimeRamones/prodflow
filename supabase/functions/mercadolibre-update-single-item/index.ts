...
serve(async (req) => {
    ...
    try {
        const { meliId, variationId, availableQuantity, sku, price } = await req.json();
        ...
        if (variationId) {
            ...
            if (availableQuantity !== undefined) {
                variationPayload.available_quantity = availableQuantity;
            }
        ...
        if (availableQuantity !== undefined) {
            ...
            updateData.prodflow_stock = availableQuantity;
        }
        if (price !== undefined) {
            updateData.price = price;
            updateData.prodflow_price = price;
        }
        ...
        if (Object.keys(updateData).length > 0) {
            const { error: updateError } = await supabaseAdmin
                ...
                .update(updateData)
                .eq('meli_id', meliId)
                .eq('meli_variation_id', variationId || null);
