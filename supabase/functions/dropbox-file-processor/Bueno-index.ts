...
// (La función getDropboxAccessToken se mantiene igual)
...
        console.log("Iniciando el procesamiento de archivos de Dropbox.");
        ...
        // (La lógica de warehouses y listado de archivos se mantiene igual)
...
            const fileContent = await downloadResponse.text();
            // Usamos una expresión regular para manejar diferentes tipos de saltos de línea (\r\n de Windows o \n de Unix)
            const lines = fileContent.split(/\r?\n/);

            const itemsToInsert = [];

            for (const line of lines) {
                // Solo usamos trim() aquí para identificar si la línea está vacía.
                if (line.trim() === '') continue;

                // --- LÓGICA DE PARSEO CORREGIDA (SEPARADO POR TABULADORES EXACTOS) ---
                
                // Dividimos la línea usando el tabulador (\t) como delimitador exacto.
                // No usamos trim() en la línea completa aquí para preservar espacios si son parte del primer o último campo.
                const parts = line.split('\t');

                // Validamos que haya al menos 3 partes (SKU, Cantidad, Precio)
                if (parts.length < 3) {
                    console.warn(`Formato inválido (No hay 3 campos separados por tabulador): "${line}"`);
                    continue;
                }

                // Extraemos las partes desde el final.
                // Usamos .trim() en cantidad y precio para eliminar posible padding dentro de esos campos.
                const priceStr = parts[parts.length - 1].trim();
                const quantityStr = parts[parts.length - 2].trim();
                
                // Extraemos el SKU (Todo lo que está antes de la cantidad y el precio).
                // Usamos slice y join('\t') por si el SKU mismo contenía un tabulador (improbable, pero robusto).
                // ¡CRUCIAL: NO USAMOS .trim() para respetar TODOS los espacios del SKU (iniciales, internos y finales)!
...
                // Aseguramos que el punto sea el separador decimal
                const cost_price = parseFloat(priceStr.replace(',', '.'));

                if (!isNaN(quantity) && !isNaN(cost_price)) {
                    itemsToInsert.push({
                        warehouse_id: warehouse.id,
                        sku: sku, // SKU exacto con todos los espacios preservados
                        quantity: quantity,
...
            }
            ...
            // (La lógica de inserción en Supabase se mantiene igual)
            if (itemsToInsert.length > 0) {
                // Borrar stock anterior del proveedor
