...
// VERSIÓN DEDICADA ÚNICAMENTE A ARCHIVOS EXCEL (.xlsx)
...
serve(async (_req) => {
    try {
        ...
        console.log("Iniciando el procesamiento de archivos EXCEL de Dropbox.");

        const { data: warehouses } = await supabaseAdmin.from('warehouses').select('id, name').eq('type', 'proveedor');
        if (!warehouses || warehouses.length === 0) {
            return new Response(JSON.stringify({ message: "No se encontraron almacenes de tipo 'proveedor'." }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        ...
        const stockFileEntries = filesData.entries.filter(file => file['.tag'] === 'file' && file.name.toLowerCase().endsWith('.xlsx'));

        for (const file of stockFileEntries) {
            const providerName = file.name.replace(/\.xlsx$/i, '');
            ...
            if (!providerConfig) {
                console.log(`Archivo Excel '${file.name}' ignorado: no hay configuración de cabeceras.`);
                continue;
            }
        ...
        return new Response(JSON.stringify({ success: true, message: "Archivos Excel de Dropbox procesados." }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }});
    } catch (error) {
