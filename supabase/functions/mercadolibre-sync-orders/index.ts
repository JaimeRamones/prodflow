...
// VERSIÓN MEJORADA: Obtiene detalles completos de cada orden

import { serve } from "https://deno.land/std@0.177.0/http/server.ts";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.42.0";
import { corsHeaders } from "../_shared/cors.ts";

// getMeliToken (sin cambios)
...
serve(async (req) => {
  ...
  try {
    ...
    for (const summaryOrder of summarizedOrders) {
      // --- NUEVO: Hacemos una segunda llamada para obtener todos los detalles ---
      ...
      if (!detailResp.ok) {
        console.warn(`No se pudieron obtener detalles para la orden ${summaryOrder.id}, se omitirá.`);
        continue;
      }
      ...
      const saleOrderData = {
        ...
        user_id: user.id,
        // Usamos los datos completos del comprador
      ...
      const orderItems = order.order_items.map(item => ({
        ...
        // Usamos la URL del thumbnail del item
    ...
    return new Response(JSON.stringify({ message: `Sincronización completada. Se procesaron ${processedCount} ventas.` }), {
