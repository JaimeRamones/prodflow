...
// VERSIÓN v20: Modo Diagnóstico (Filtro Óptimo + Arquitectura Optimizada).
...
// Añadir aquí los SKUs exactos (Kits/Premium) que se desean rastrear.
const SKUS_TO_TRACE = ['HG31101/X2', 'SE31057/X2']; // <--- AÑADIR SKUs AQUÍ
// ------------------------------
...
serve(async (_req) => {
  try {
    console.log(`Iniciando PROCESADOR DE KITS v20 (Modo Diagnóstico: ${DIAGNOSTIC_MODE ? 'ACTIVADO' : 'Desactivado'})...`);
    ...
    while (continueProcessing) {
        ...
        for (const listing of listingsBatch) {
            ...
            if (!baseSku) {
                 if (isTracing) console.log(`--- TRACE: Omitido. Motivo: Es un producto Base (manejado por stock-aggregator). ---\n`);
                 continue;
            }
            
            if (isTracing) console.log(`TRACE: Tipo detectado: ${isPremium ? 'Premium' : 'Kit'}. SKU Base: ${baseSku}`);
            ...
            if (!baseData || !baseData.baseCost || !rules.defaultMarkup) {
                skippedCount++;
                 if (isTracing) console.log(`--- TRACE: Omitido. Motivo: Falta Costo Base o Markup en el SKU Base. Costo:${baseData?.baseCost}, Markup:${rules.defaultMarkup} ---\n`);
                continue;
            }
            ...
            if (expectedPrice < MINIMUM_PRICE && expectedPrice > 0) {
             ...
             if (isTracing) {
                console.log(`TRACE: Cálculos Base -> Costo:${baseData.baseCost}, Stock Agregado:${baseData.stock}`);
    ...
    return new Response(JSON.stringify({ success: true, message: `Sincro de KITS (v20) completada. Actualizados: ${updatedCount}. Fallidos: ${failedCount}.` }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }});
