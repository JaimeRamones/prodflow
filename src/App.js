...
const AppProvider = ({ children }) => {
    ...
    const value = { session, loading, showMessage, products, suppliers, categories, kits, salesOrders, supplierOrders, purchaseOrders, notification, setNotification, fetchSuppliers, fetchCategories, fetchProducts, fetchKits, fetchSalesOrders, fetchSupplierOrders, fetchPurchaseOrders };
...
const AppContent = () => {
    ...
    const NavButton = ({ tabName, iconPath, children }) => ( <li><button onClick={() => setActiveTab(tabName)} className={`flex items-center w-full p-2 text-base font-normal rounded-lg transition duration-75 group ${ activeTab === tabName ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-700 hover:text-white' }`}><Icon path={iconPath} /><span className="flex-1 ml-3 whitespace-nowrap">{children}</span></button></li> );
        ...
        <div className="bg-gray-900 text-gray-300 min-h-screen">
            ...
            <aside className="fixed top-0 left-0 z-40 w-64 h-screen transition-transform bg-gray-800 border-r border-gray-700 sm:translate-x-0">
                <div className="h-full px-3 py-4 overflow-y-auto bg-gray-800">
                    <div className="flex items-center pl-2.5 mb-5"><span className="self-center text-xl font-semibold whitespace-nowrap text-white ml-3">ProdFlow</span></div>
                    ...
                    <ul className="pt-4 mt-4 space-y-2 border-t border-gray-700">
                        ...
                        <li><button onClick={handleLogout} className="flex items-center w-full p-2 text-base font-normal text-gray-400 rounded-lg transition duration-75 group hover:bg-red-800 hover:text-white"><Icon path="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /><span className="flex-1 ml-3 whitespace-nowrap">Cerrar Sesión</span></button></li>
                    </ul>
                </div>
            </aside>
            <main className="p-4 sm:ml-64"><div className="mt-14">{renderActiveTab()}</div></main>
...
const AppOrchestrator = () => {
    ...
    if (loading) {
        return <div className="min-h-screen flex items-center justify-center bg-gray-900"><p className="text-xl font-semibold text-white">Cargando...</p></div>;
    }
