...
// VERSIÓN CON LÓGICA DE IMPRESIÓN PDF ACTIVADA
...
const SalesView = () => {
    ...
    const [isPrinting, setIsPrinting] = useState(false); // <-- NUEVO ESTADO
    ...
    // --- FUNCIÓN DE IMPRESIÓN MODIFICADA Y FUNCIONAL ---
    const handlePrintLabels = async (format) => {
        ...
        setIsPrinting(true);
        showMessage(`Generando ${selectedOrders.size} etiqueta(s) en formato ${format}...`, 'info');
        ...
        try {
            for (const orderId of selectedOrders) {
                ...
                a.href = url;
                a.download = `etiqueta-${order.meli_order_id}.${format}`;
    ...
    return (
        <div>
            ...
            {/* Botones de acción masiva actualizados */}
            ...
            {/* ... (El resto del JSX para mostrar las órdenes no cambia) ... */}
