...
// VERSIÓN FINAL: Mantiene la lógica de etiquetas ZPL funcional y corrige las imágenes rotas.

import React, { useState, useEffect, useContext, useMemo } from 'react';
import { AppContext } from '../App';
import { supabase } from '../supabaseClient';
import ImageZoomModal from './ImageZoomModal';
import JSZip from 'jszip'; // La librería JSZip es necesaria para la lógica correcta de ZPL.
...
const SalesView = () => {
    ...
    const processedOrders = useMemo(() => {
        ...
        const enriched = salesOrders.map(order => ({
            ...
            order_items: order.order_items.map(item => {
                ...
                // --- CORRECCIÓN FINAL PARA IMÁGENES ---
                // 1. Aseguramos que la miniatura sea HTTPS
                const secureThumbnail = item.thumbnail_url ? item.thumbnail_url.replace(/^http:/, 'https:') : null;
                // 2. Aseguramos que TODAS las URLs de la tabla de productos también sean HTTPS
    ...
    const handlePrintLabels = async (format) => {
        ...
        try {
            ...
            // Lógica correcta para ZPL (usa JSZip) y PDF
            if (format === 'zpl') {
                const zip = new JSZip();
