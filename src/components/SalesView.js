    ...
    // --- FUNCIÓN DE IMPRESIÓN (Usando 'get-ml-labels' Edge Function) ---
     const handlePrintLabels = async (format) => {
        ...
        try {
            ...
            // Usamos supabase.functions.invoke configurado para Blob
            ...
            <div className="bg-gray-800 border border-gray-700 p-4 rounded-lg mb-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <select value={filters.status} onChange={e => setFilters({...filters, status: e.target.value})} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md text-white">
                        <option value="all">Todos los Estados (Interno)</option>
                        <option value="Recibido">Recibido (Sin Procesar)</option>
                        ...
                        <option value="Despachado">Despachado</option>
                         <option disabled>--- ESPECIALES ---</option>
                        <option value="daily_dispatch">Envíos del Día</option>
                        <option value="cancelled">Canceladas</option>
                    </select>
                     <select value={filters.printStatus} onChange={e => setFilters({...filters, printStatus: e.target.value})} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md text-white">
                        <option value="all">Estado Etiqueta (ML)</option>
                        <option value="pending_print">Sin Imprimir (Listo p/ preparar)</option>
                        <option value="printed">Impresa (Listo p/ colecta)</option>
                    </select>
                </div>
            </div>
