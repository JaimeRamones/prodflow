...
const SalesView = () => {
    ...
    // --- LÓGICA DE IMPRESIÓN EN EL CLIENTE ---
    const handlePrintLabels = async (format) => {
        ...
        setIsPrinting(true);
        if (format === 'zpl') { showMessage("ZPL no implementado.", "warning"); setIsPrinting(false); return; }

        try {
            for (const orderId of selectedOrders) {
                const { data: labelData, error } = await supabase.functions.invoke('get-ml-labels', { body: { order_id: orderId } });
                ...
                page.drawText(`${receiverAddress.receiver_name || 'Nombre no disponible'}`, { x: 20, y: height - 75, font: fontBold, size: 12 });
                page.drawText(`${receiverAddress.address_line || 'Dirección no disponible'}`, { x: 20, y: height - 90, font, size: 10 });
                ...
                for (const item of items) {
                    const stockIndicator = item.has_stock ? "[EN STOCK]" : "[PEDIR]";
                    page.drawText(`${item.quantity}x ${item.sku}`, { x: 20, y: yPosition, font: fontBold, size: 10 });
                    page.drawText(stockIndicator, { x: width - 70, y: yPosition, font: fontBold, size: 10, color: item.has_stock ? rgb(0, 0.5, 0) : rgb(0.8, 0, 0) });
                    yPosition -= 12;
                }
                ...
                a.href = url;
                a.download = `etiqueta-custom-${order.meli_order_id}.pdf`;
