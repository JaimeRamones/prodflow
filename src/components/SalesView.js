            ...
            if (error) {
                ...
                const enrichedOrders = data.map(order => ({
                    ...
                    order_items: order.order_items.map(item => {
                        ...
                        const costWithVat = productInfo?.cost_price ? (productInfo.cost_price * 1.21).toFixed(2) : 0;
                        return {
                            ...item,
            ...
            <div className="space-y-4">
                {isLoading ? ( <p className="text-center p-8 text-gray-400">Cargando ventas...</p> ) : (
                    orders.length > 0 ? orders.map(order => (
                        <div key={order.id} className="bg-gray-800 border border-gray-700 rounded-lg shadow-lg flex gap-4 p-4">
                                    ...
                                    {order.order_items.map((item, index) => (
                                        <div key={index} className="flex items-start gap-4">
                                            <div className="flex-shrink-0 flex gap-2">
                                                ...
                                                {item.images[1] && <img src={item.images[1]} alt={item.title} className="hidden md:block w-16 h-16 object-cover rounded-md border border-gray-600 cursor-pointer" onClick={() => setZoomedImageUrl(item.images[1])}/>}
                                            </div>
                                            <div className="flex-grow">
